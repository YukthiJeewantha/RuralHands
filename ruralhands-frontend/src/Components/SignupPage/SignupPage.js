...
const handleSubmit = async () => {
  ...
  try {
    const response = await fetch("http://localhost:5000/api/register/buyer", {
      method: "POST",
      headers: {
      ...
      <div className="w-full max-w-md bg-white/95 backdrop-blur-sm rounded-2xl shadow-xl border border-gray-100 p-8 relative z-10">
        ...
        <div className="mt-6 text-center">
          <p className="text-gray-600 text-sm">
            ...
            <button onClick={() => navigate('/LoginPage')} className="text-blue-600 hover:text-blue-700 font-medium transition-all duration-200 cursor-pointer hover:scale-105 hover:underline hover:bg-blue-50 px-2 py-1 rounded">
              Sign in
            </button>
          </p>
        </div>
